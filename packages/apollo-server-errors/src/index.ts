...
export class AuthenticationError extends ApolloError {
  constructor(message: string) {
    super(message, 'UNAUTHENTICATED', undefined, 'UNAUTHENTICATED');
...
export function formatApolloErrors(
  ...
  const makePrintable = error => {
    function parseApolloError(parsedError: ApolloError, originalError: GraphQLFormattedError) {
      return {
        message: parsedError.message,
        errorType: parsedError.errorType,
        ...(originalError.locations && { locations: originalError.locations }),
        ...(originalError.path && { path: originalError.path }),
        ...(originalError.extensions && { extensions: originalError.extensions }),
      };
    }
    ...
    if(error instanceof GraphQLError) {
      ...
      if(graphQLError.originalError instanceof ApolloError) {
        return parseApolloError(graphQLError.originalError, graphQLError);
      }
    }
    ...
    if (error instanceof Error) {
      ...
      console.trace("Undefined error Type detected. Investigate the code to properly wrap it to ApolloError", graphQLError);
      return {
